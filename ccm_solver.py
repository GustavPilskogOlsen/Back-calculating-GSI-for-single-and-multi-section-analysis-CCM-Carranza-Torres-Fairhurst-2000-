...
def _rrs_parallel(R: float, *, tc_mm: float, Ec_MPa: float, nu_c: float,
    ...
    """Shotcrete shell + steel ring in parallel — identical to the Excel
    workbook (cells F99/F101/F102 + L102/L103/L104 + O102/O103/O104).

    The shotcrete shell uses the Lamé thick-cylinder solution (L102, L103,
    L104). The steel ring uses the classic axisymmetric thin-ring formulas
    (O102, O103, O104):
        K_s     = E_s * A_s / (S_l * R^2)            [MPa/m]
        p_s_max = sigma_ys * A_s / (S_l * R)         [MPa]
        u_s_max = sigma_ys * R / E_s * 1000          [mm]
    These formulas treat the steel ribs as discrete circumferential rings
    loaded radially without Poisson coupling. The parameter ``nu_s`` is
    kept in the function signature for API compatibility but is not used,
    matching the Excel implementation.
    """
    # Shotcrete shell (Lamé) — identical to Excel L102/L103/L104.
    p_c_max, K_c, u_c_max = _shotcrete(R, sigma_cc, Ec_MPa / 1000.0, nu_c, tc_mm)

    # Steel ring — identical to Excel O102/O103/O104.
    A_s = nbars * pi * (dbar_mm / 1000.0) ** 2 / 4.0        # bar area [m^2]
    K_s = (Es_MPa * A_s) / (Sl * R ** 2)                     # MPa/m
    p_s_max = (sigma_ys * A_s) / (Sl * R)                    # MPa
    u_s_max = sigma_ys * R / Es_MPa * 1000.0                 # mm
